"""Fourier computation engine for Experiment 06."""
...
class FourierSensitivityEngine:
    """Compute Fourier coefficients and sensitivity metrics for periodic signals."""
    ...
    def evaluate(self, x: np.ndarray, function_name: str, amplitude: float = 1.0) -> np.ndarray:
        base = self.get_function(function_name)(x)
        return amplitude * base

    def compute_coefficients(self, function_name: str, n_terms: int, n_samples: int = 20000, amplitude: float = 1.0) -> dict:
